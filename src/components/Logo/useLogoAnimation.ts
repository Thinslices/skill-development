...
export const useLogoAnimation = ( wrapperRef: LogoElement, thinslicesRef: LogoElement, knowledgeRef: LogoElement, iconRef: LogoElement ) => {
    const [ source, setSource ] = useState<HTMLDivElement>();
    const delay = useRef<number>( 0 );

    const changeSource = useCallback( () => {
        const nextSource = source === thinslicesRef.current ? knowledgeRef.current : thinslicesRef.current;
    ...
    useLayoutEffect( () => {
        if ( wrapperRef.current && iconRef.current && knowledgeRef.current && thinslicesRef.current && iconRef.current && source ) {
            ...
            iconRef.current.style.translate = `${ thinslicesRef.current.offsetWidth - source.offsetWidth }px 0`;
            iconRef.current.style.transition = 'all 1s cubic-bezier(0.68, -0.6, 0.32, 1.6)'
